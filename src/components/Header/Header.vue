<template>
    ...
    <nav id="nav-links-grid"  v-if="!isMobileView" class="absolute right-0 top-0 h-full flex items-center py-2">
      <ul
        ...
        :class="[
          ...
          isTabletView ? 'space-x-2 text-sm' : 'space-x-4',
        ]"
      >
        <li id="home-item">
          <router-link
            to="/home"
            class="px-3 pb-2 hover:text-[#087bb4] hover:border-b-2 border-[#087bb4]"
            >Home</router-link
          >
        </li>
        <li id="about-item">
      ...
      <div class="border-t py-6 px-6">
        <div id="mobile-nav-socials" class="flex justify-around">
          ...
          <a
            ...
            target="_blank"
            class="w-6 h-6 hover:text-[#087bb4]"
            aria-label="Youtube"
          >
            <box-icon
              type="logo"
  ...
  <div
    v-if="showAlert"
    class="flex justify-between w-1/4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 z-50 rounded fixed bottom-5 right-5 transition-opacity duration-300 ease-in-out transform"
    role="alert"
  >
    <span class="block sm:inline"
      ><strong class="font-bold">Log out</strong> successful!</span
    >
